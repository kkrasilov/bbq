  ...
  describe '.find_for_facebook_oauth' do
    ...
    let(:access_token) do
      double(
        :access_token,
        provider: 'facebook',
        info: double(email: 'kkrasilov@gmail.com', name: 'konstantin',),
    ...
    context 'when user is not found' do
      it 'returns newly created user' do
        ...
        expect(user.email).to eq 'kkrasilov@gmail.com'
      end
    end

    # Ситуация: юзер найден по почте
    context 'when user is found by email' do
      let!(:existing_user) { create(:user, email: 'kkrasilov@gmail.com') }
